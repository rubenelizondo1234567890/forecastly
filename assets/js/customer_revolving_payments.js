/* assets/js/customer_revolving_payments.js */
...
function handleFormSubmit(e) {
    ...
    const chosenAmountField = document.querySelector('[name="revolving_payments[chosenAmount]"]');
    const dayOfMonthField = document.querySelector('[name="revolving_payments[dayOfMonthToMakePayment]"]');
    ...
    if (strategy === 'Min. Payment (2%)') {
        // Set chosenAmount to null for minimum payment strategy
    ...
    // Validate percentage if strategy is Min. Payment + percentage
    if (strategy === 'Min. Payment (2%) + given % of projected balance' && chosenAmountField) {
        const percentage = parseFloat(chosenAmountField.value);
        if (isNaN(percentage) || percentage <= 0) {
            alert('Please select a valid percentage for the additional payment.');
            return false;
        }
    }
    ...
    // Get form data
    const form = document.getElementById('revolving-payments-form');
    ...
    fetch(form.action, {
        ...
        .then(data => {
            if (data.success) {
                ...
                const accountToWithdraw = document.querySelector('[name="revolving_payments[accountToWithdraw]"]');
                const accountToPay = document.querySelector('[name="revolving_payments[accountToPay]"]');
                ...
                const payId = data.accountToPayId || accountToPay.value;

                if (withdrawId && payId) {
                    return updateBothAccountsInterest(withdrawId, payId)
                        .then(() => {
                            hideLoadingModal();
                            ...
                            setTimeout(() => {
                                window.location.href = '/customer/revolving-payments/';
                            }, 1500);
                        })
                        .catch(error => {
...
function updateBothAccountsInterest(accountToWithdrawId, accountToPayId) {
    const promises = [];
    ...
    if (accountToPayId) {
        promises.push(updateAccountInterest(accountToPayId));
    }
...
// Handle payment strategy changes
function handleStrategyChange() {
    ...
    switch(strategy) {
        ...
        case 'Min. Payment (2%) + given % of projected balance':
            ...
            if (amountHint) {
                amountHint.textContent = 'Select the additional percentage of the projected balance to add to the minimum payment';
            }
        ...
        case 'Fixed Amount':
            amountContainer.style.display = 'block';
            if (amountHint) {
                amountHint.textContent = 'Enter the fixed amount to pay each month';
            }
...
function updateFieldToDropdown(wrapper) {
    ...
    select.name = 'revolving_payments[chosenAmount]';
...
function updateFieldToNumberInput(wrapper) {
    ...
    input.type = 'number';
    input.name = 'revolving_payments[chosenAmount]';
...
function handleDeleteClick(e) {
    if (!confirm('Are you sure you want to delete this revolving payment?')) {
        e.preventDefault();
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (strategySelect) {
        ...
        if (currentStrategy && currentStrategy !== 'Min. Payment (2%)') {
            handleStrategyChange();
        }
    }
