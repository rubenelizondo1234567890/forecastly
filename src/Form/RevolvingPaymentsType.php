...
class RevolvingPaymentsType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ...
            ->add('paymentStrategy', ChoiceType::class, [
                'required' => true,
                'choices' => [
                    'Min. Payment (2%)' => 'Min. Payment (2%)',
                    'Min. Payment (2%) + given % of projected balance' => 'Min. Payment (2%) + given % of projected balance',
            ...
            ->add('dayOfMonthToMakePayment', ChoiceType::class, [
                'required' => true,
                'choices' => $this->getDayOfMonthChoices(),
                'attr' => ['class' => 'form-control'],
                'label' => 'Day of Month to Make Payment',
            ...
            ->add('canceledAfter', DateType::class, [
                'widget' => 'single_text',
                'required' => false,
                'attr' => ['class' => 'form-control'],
                'label' => 'Cancel After Date (optional)'
            ])
            ->add('accountToPay', EntityType::class, [
                'class' => Account::class,
                'choice_label' => function (Account $account) {
                    return $account->getName() . ' -- ' . $account->getBudgetTrackingGroup()->getIsIncomeOrExpense();
                },
                'attr' => ['class' => 'form-select'],
                'label' => 'Account to apply monthly payments',
                'query_builder' => function (AccountRepository|EntityRepository $er) use ($options) {
                    return $er->createQueryBuilder('a')
                        ...
                        ->andWhere('b.isIncomeOrExpense = :expenseType')
        ...
        $builder->addEventListener(FormEvents::POST_SET_DATA, function (FormEvent $event) {
            ...
            $strategy = $data ? $data->getPaymentStrategy() : null;
        ...
        $builder->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $event) {
            ...
            $strategy = $data['paymentStrategy'] ?? null;
    ...
    private function updateChosenAmountField($form, ?string $strategy, bool $isPreSubmit = false)
    {
        ...
        if ($strategy === 'Min. Payment (2%) + given % of projected balance') {
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            'data_class' => RevolvingPayments::class,
