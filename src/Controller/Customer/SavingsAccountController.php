...
// src/Controller/Customer/SavingsAccountController.php
...
#[Route('/customer/savings-account', name: 'customer_savings_account_')]
...
class SavingsAccountController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, AccountsServiceInterface $accountsService): Response
    {
        ...
        $form = $this->createForm(SavingsAccountType::class, $account, [
            'customerAccount' => $cAcct,
            'action' => $this->generateUrl('customer_savings_account_new')
        ]);
        ...
        if ($form->isSubmitted() && ($form->isValid() || (!$form->isValid() && json_encode($form->getErrors()) == '{}'))) {
            ...
            $this->addFlash('success', 'Savings Account created successfully');
            if ($account->getAccountType()) {
                $this->addFlash('success', 'Recurring Monthly Interest for this Account has been created!');
            }

            return $this->redirectToRoute('customer_savings_account_index');
        }
    ...
    public function edit(Request $request, Account $account, EntityManagerInterface $entityManager, AccountsServiceInterface $accountsService): Response
    {
        ...
        if ($form->isSubmitted() && ($form->isValid() || (!$form->isValid() && json_encode($form->getErrors()) == '{}'))) {
            ...
            $this->addFlash('success', 'Savings Account updated successfully');
        ...
        foreach ($accounts as $account) {
            if ($account->getBudgetTrackingGroup()->getIsIncomeOrExpense() == 'asset') {
                //Get the projected Balance from Accounts Tracking Calendar that corresponds to today
                $entry = $entityManager->getRepository(AccountsTrackingCalendar::class)->findOneBy(['customersAccount' => $cAcct, 'calendarDate' => $today]);
                if ($entry) {
                    $accountsBalances = $entry->getAccountsBalances();
        ...
        return $this->render('customer/savings_account/index.html.twig', [
            'accounts' => $results ?? [],
            'account' => $account,
    ...
    public function index(EntityManagerInterface $entityManager): Response
    {
        ...
        $form = $this->createForm(SavingsAccountType::class, $newAccount, [
            'customerAccount' => $cAcct,
            'action' => $this->generateUrl('customer_savings_account_new')
        ]);
    ...
    private function createAdjustment(Account $account, float $amount, string $type): NonRecurringIncomeExpensesInterface
    {
        ...
        $raName = 'SA-ACCTID-' . $account->getId() . '-' . $type . '-' . $account->getLastReconciliationDate()->format('Y-m-d');
